  ...
  try {
    ...
    for (let meter of meters) {
      ...
      volatageMeter.push({
        ...
        totalReactPower: len2Data[5],
        totalPF: len2Data[6],
        unbalVoltig: len2Data[7],
        unbalCurrent: len2Data[8],
        importActPower: len2Data[9],
        importReactPower: len2Data[10],
        exportActPower: len2Data[11],
        exportReactPower: len2Data[12],
        time: len2Data[13],
      });
      await sleep(100);
    }
...
const getMeterValueLen2 = async (id) => {
  const addresses = [59, 61, 223, 63, 65, 67, 69, 71, 73, 501, 503, 505, 507, 799,]; //all addresses for len2
...
async function main() {
  ...
  const combined = totalPowerData.map((item) => [
    ...
    item.totalAppPower,
    item.totalActPower,
    item.totalReactPower,
    item.totalPF,
    item.unbalVoltig,
    item.unbalCurrent,
    item.importActPower,
    item.importReactPower,
    item.exportActPower,
    item.exportReactPower,
    item.time,
  ]);
